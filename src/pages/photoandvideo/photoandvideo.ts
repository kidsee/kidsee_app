...
export class PhotoAndVideoPage {
  ...
  protected goToPost(post: Post) {
    this.navController.push('PhotoPostPage', {post: post});
  }
  ...
  private fetchNewPage() {
    ...
    this.contentTypeService.getTypeByName("image").then(contentType => {
      ...
      this.postService.postsByTheme(this.selectedFilter, params).subscribe(data => {
        this.authService.fetchCurrentUser().then(user => {
          ...
          posts.forEach(post => {
            ...
            post.content = this.pictureService.retrieveFullImageUrl(post.content);
  ...
  protected createPost(){
    this.navController.push('CreatePhotoPage', this.location);
  }
